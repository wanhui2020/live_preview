...
class DealChatRepository extends BaseRepository
{
    public function model()
    {
        return DealChat::class;
    }
    ...
    public function store(array $data)
    {
        ...
        try {
            ...
            if ($goldFrom->usable < $chat->total) {
                DB::rollBack();
                return $this->validation('金币不足，请充值');
            }
            $goldFrom->usable = $goldFrom->usable - $chat->total;
            if ($goldFrom->lock >= $chat->total) {
                $goldFrom->lock = $goldFrom->lock - $chat->total;
            }
            $goldFrom->save();
            //会员聊天解锁支出
            $goldFrom->records()->save(new MemberWalletRecord(['type' => 47, 'member_id' => $chat->member_id, 'money' => -$chat->total, 'surplus' => $goldFrom->balance]));

            //判断是否有代理
            $agentId = MemberUser::where(['id' => $chat->member_id, 'is_middleman' => 1])->value('agent_id');
            $middlemanRechargeRate = MemberUserRate::where(['member_id' => $agentId])->value('middleman_recharge_rate');
            if ($agentId && $middlemanRechargeRate > 0) {
                //返回收入激励的
                $money = bcmul($chat->total, $middlemanRechargeRate, 2);
                ...
                $goldFrom->records()->save(new MemberWalletRecord(['type' => 20, 'member_id' => $agentId, 'money' => $money, 'surplus' => $memberWalletGold->balance]));
