    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('system/platform/notice/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '公告管理'
                , cols: [[
                    ...
                    {field: 'name', title: '标题' }
                    , {
                        field: 'type', title: '通知类型', align: 'center',width: 120,
                        templet: function (d) {
                            if (d.type === 0) {
                                return '系统';
                            }
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '通知发布'
                            , content: '{{url('system/platform/notice/create')}}'
                            , maxmin: true
                            , area: ['800px', '600px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) {
                ...
                if (layEvent === 'status') { //改变状态
                    ...
                    layer.confirm('确定要【' + _c + '】么?', {icon: 3, btn: ['确定', '取消'], title: "信息提示"}, function (index) {
                        axios.post("{{url('system/platform/notice/status')}}", {id: data.id})
                    ...
                    layer.open({
                        type: 2
                        , title: '编辑公告'
                        , content: '{{url('system/platform/notice/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['800px', '600px']
                        , end: function () {
                            tableIns.reload();
                    ...
                    layer.confirm('确定要删除【' + data.name + '】么？', {
                        ...
                        axios.post("{{url('system/platform/notice/destroy')}}", {ids: [data.id]})
                            .then(function (response) {
