    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('system/platform/payment/channel/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '通道管理'
                , cols: [[
                    ...
                    , {
                        field: 'icon', title: '通道图标', align: 'center', width: 120,
                        templet: function (d) {
                            if (d.icon) {
                                return '<img src="' + d.icon + '" alt="" width="50px" height="50px">';
                            }
                            return '无';
                        }
                    }, {field: 'name', title: '通道名称', align: 'center'}
                    , {field: 'code', title: '通道标识', align: 'center'}
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '添加支付通道'
                            , content: '{{url('system/platform/payment/channel/create')}}'
                            , maxmin: true
                            , area: ['800px', '600px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) {
                ...
                if (layEvent === 'status') { //状态
                    layer.confirm('确定要修改状态?', {icon: 3, btn: ['确定', '取消'], title: "信息提示"}, function (index) {
                        axios.post("{{url('system/platform/payment/channel/status')}}", {id: data.id})
                ...
                if (layEvent === 'edit') { //编辑
                    layer.open({
                        type: 2
                        , title: '编辑公告'
                        , content: '{{url('system/platform/payment/channel/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['900px', '800px']
                        , end: function () {
                            tableIns.reload();
                    ...
                    layer.confirm('确定要删除么？', {
                        ...
                        axios.post("{{url('system/platform/payment/channel/destroy')}}", {ids: [data.id]})
                            .then(function (response) {
