    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('system/platform/price/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '充值价格维护'
                , cols: [[
                    {field: 'id', title: 'ID', align: 'center',width:80}, {field: 'name', title: '价格名称', align: 'center'}
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '添加价格类型'
                            , content: '{{url('system/platform/price/create')}}'
                            , maxmin: true
                            , area: ['800px', '600px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) {
                ...
                if (layEvent === 'status') { //改变状态
                    ...
                    layer.confirm('确定要【' + _c + '】么?', {icon: 3, btn: ['确定', '取消'], title: "信息提示"}, function (index) {
                        axios.post("{{url('system/platform/price/status')}}", {id: data.id})
                    ...
                    layer.open({
                        type: 2
                        , title: '编辑价格维护'
                        , content: '{{url('system/platform/price/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['900px', '800px']
                        , end: function () {
                            tableIns.reload();
                    ...
                    layer.confirm('确定要删除么？', {
                        ...
                        axios.post("{{url('system/platform/price/destroy')}}", {ids: [data.id]})
                            .then(function (response) {
