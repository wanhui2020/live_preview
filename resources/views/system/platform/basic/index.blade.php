                ...
                <div class="layui-form-item">
                    <div class="layui-inline">
                        <div class="layui-input-inline">
                            <input type="text" name="key" placeholder="Key/Value" autocomplete="off"
                                   class="layui-input">
                        </div>
                    </div>
                    <div class="layui-inline">
    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('system/platform/basic/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '管理'
                    ...
                    {field: 'id', title: 'ID', align: 'center', width: 80}, {
                        ...
                        templet: function (d) {
                            ...
                            if (d.type === 'report') {
                                ...
                                html = '收入'
                            } else if (d.type === 'profession') {
                                html = '职业'
                            } else if (d.type === 'hobbies') {
                                html = '兴趣爱好'
                            }
                            return html;
                        }
                    }
                    , {field: 'key', title: 'Key标签', width: 180, align: 'center'}
                    , {field: 'value', title: 'Value值', align: 'center'}
                    , {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '添加基础数据'
                            , content: '{{url('system/platform/basic/create')}}'
                            , maxmin: true
                            , area: ['800px', '500px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) {
                ...
                if (layEvent === 'status') { //改变状态
                    ...
                    layer.confirm('确定要【' + _c + '】么?', {icon: 3, btn: ['确定', '取消'], title: "信息提示"}, function (index) {
                        axios.post("{{url('system/platform/basic/status')}}", {id: data.id})
                    ...
                    layer.open({
                        type: 2
                        , title: '编辑公告'
                        , content: '{{url('system/platform/basic/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['800px', '500px']
                        , end: function () {
                            tableIns.reload();
                    ...
                    layer.confirm('确定要删除么？', {
                        ...
                        axios.post("{{url('system/platform/basic/destroy')}}", {ids: [data.id]})
                            .then(function (response) {
