    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('system/platform/vip/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: 'vip管理'
                , cols: [[
                    ...
                    {field: 'name', title: 'VIP名称', align: 'center'}
                    , {field: 'grade', title: '等级', align: 'center'}
                    , {field: 'integral', title: 'VIP积分', align: 'center'}
                    , {field: 'price', title: '直升机价格', align: 'center'}
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '添加VIP'
                            , content: '{{url('system/platform/vip/create')}}'
                            , maxmin: true
                            , area: ['900px', '800px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) {
                ...
                if (layEvent === 'status') { //改变状态
                    ...
                    layer.confirm('确定要【' + _c + '】么?', {icon: 3, btn: ['确定', '取消'], title: "信息提示"}, function (index) {
                        axios.post("{{url('system/platform/vip/status')}}", {id: data.id})
                    ...
                    layer.open({
                        type: 2
                        , title: '编辑VIP'
                        , content: '{{url('system/platform/vip/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['900px', '800px']
                        , end: function () {
                            tableIns.reload();
                    ...
                    layer.confirm('确定要删除么？', {
                        ...
                        axios.post("{{url('system/platform/vip/destroy')}}", {ids: [data.id]})
                            .then(function (response) {
