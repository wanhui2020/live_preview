    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('system/platform/type/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '首页类型管理'
                , cols: [[
                    ...
                    , {
                        field: 'status', title: '类型', align: 'center', width: 80,
                        templet: function (d) {
                            let html='';
                            if (d.type === 0) {
                                return "首页";
                    ...
                    , {
                        field: 'is_system', title: '是否系统', align: 'center', width: 100,
                        templet: function (d) {
                            let html='';
                            if (d.is_system === 0) {
                                return "否";
                    ...
                    , {field: 'sort', title: '序号', width: 100, sort: true, align: 'center'}
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '添加'
                            , content: '{{url('system/platform/type/create')}}'
                            , maxmin: true
                            , area: ['800px', '600px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) {
                ...
                if (layEvent === 'status') { //改变状态
                    ...
                    layer.confirm('确定要【' + _c + '】么?', {icon: 3, btn: ['确定', '取消'], title: "信息提示"}, function (index) {
                        axios.post("{{url('system/platform/type/status')}}", {id: data.id})
                    ...
                    layer.open({
                        type: 2
                        , title: '编辑'
                        , content: '{{url('system/platform/type/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['800px', '600px']
                        , end: function () {
                            tableIns.reload();
                    ...
                    layer.confirm('确定要删除【' + data.name + '】么？', {
                        ...
                        axios.post("{{url('system/platform/type/destroy')}}", {ids: [data.id]})
                            .then(function (response) {
