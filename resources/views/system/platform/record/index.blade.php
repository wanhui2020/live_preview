    ...
    <div class="layui-fluid">
        <div class="layui-card">
                ...
                <div class="layui-form-item">
                    <div class="layui-inline">
                        <div class="layui-input-inline">
                            <input type="text" name="key" placeholder="手机号/标题/姓名/内容" autocomplete="off"
                                   class="layui-input">
                        </div>
                    </div>
                    <div class="layui-inline">
            ...
            <div class="layui-card-body">
                ...
                {{--<div>--}}
                {{--<button class="layui-btn  layui-btn-sm" lay-event="add">发送短信</button>--}}
                {{--</div>--}}
                {{--</script>--}}
    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('system/platform/record/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '短信记录'
                ...
                , cols: [[
                    {field: 'id', title: 'ID', align: 'center',width:80}, {field: 'phone', title: '手机号', align: 'center', width: 120}
                    , {field: 'name', title: '姓名', align: 'center', width: 100}
                    , {field: 'title', title: '标题', align: 'center', width: 120}
                    , {field: 'content', title: '内容', align: 'center'}
                    , {field: 'created_at', title: '发送时间', width: 200, sort: true, align: 'center'}
                    , {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '发送短信'
                            , content: '{{url('system/platform/record/create')}}'
                            , maxmin: true
                            , area: ['800px', '700px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) {
                ...
                if (layEvent === 'rates') { //倍数费率
                    ...
                    layer.confirm('确定要【' + _c + '】么?', {icon: 3, btn: ['确定', '取消'], title: "信息提示"}, function (index) {
                        axios.post("{{url('system/platform/rate/status')}}", {id: data.id})
                            .then(function (response) {
                    ...
                    layer.open({
                        type: 2
                        , title: '编辑倍数费率'
                        , content: '{{url('system/platform/rate/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['800px', '700px']
                        , end: function () {
                            tableIns.reload();
                    ...
                    layer.confirm('确定要删除【' + data.name + '】么？', {
                        ...
                        axios.post("{{url('system/platform/record/destroy')}}", {ids: [data.id]})
                            .then(function (response) {
                                    if (response.data.status) {
