    ...
    <script type="application/javascript">
        ...
        layui.use(['table'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('system/deal/gold/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '能量充值'
                , cols: [[
                    ...
                    , {field: 'money', title: '产品价格', align: 'center'}
                    , {field: 'gold', title: '充值能量', align: 'center'}
                    , {field: 'give', title: '赠送能量', align: 'center'}
                    , {field: 'received', title: '实到能量', align: 'center'}
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '能量充值'
                            , content: '{{url('system/deal/gold/create')}}'
                            , maxmin: true
                            , area: ['800px', '400px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) {
                ...
                if (layEvent === 'cancel') { //取消
                    layer.confirm('确认取消订单', function (index) {
                        axios.post("{{url('system/deal/gold/cancel')}}", {id: data.id })
                ...
                if (layEvent === 'pay') { //支付
                    layer.confirm('确认支付', function (index) {
                        axios.post("{{url('system/deal/gold/pay')}}", {id: data.id})
                            .then(function (response) {
                                    if (response.data.status) {
                                        layer.msg(response.data.msg);
                                        return tableIns.reload();
                ...
                if (layEvent === 'del') { //删除
                    layer.confirm('真的删除么', function (index) {
                        axios.post("{{url('system/deal/gold/destroy')}}", {ids: [data.id]})
                            .then(function (response) {
                                    if (response.data.status) {
                                        layer.msg(response.data.msg);
                                        return tableIns.reload();
