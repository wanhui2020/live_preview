        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <div class="layui-form-item">
                    ...
                    <div class="layui-inline">
                        <div class="layui-input-inline">
                            <input type="text" class="layui-input" name="dateTime" id="test" placeholder="申请时间"
                                   lay-key="17">
                        </div>
                    </div>
                    <div class="layui-inline">
    ...
    <script type="application/javascript">
        ...
        layui.use(['table'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('system/member/user/selfie/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '会员管理'
                , cols: [[
                    ...
                    , {
                        field: 'picture', title: '自拍照', align: 'center',
                        templet: function (d) {
                            if (d.picture) {
                                return '<div onclick="show_img(this)" ><img src="' + d.fill_picture + '" alt="" width="500px" height="50px"></a></div>';
                            } else {
                                return '无';
                            }
                        }
                    ...
                    , {
                        field: 'video', title: '自拍视频', align: 'center',
                        templet: function (d) {
                            if (d.video) {
                                // return '<div onclick="show_img(this)" ><img src="' + d.fill_video + '" alt="" width="500px" height="50px"></a></div>';
                                return '<a href="' + d.fill_video + '" target="_blank" title="点击查看"><img src="' + d.fill_video + '?x-oss-process=video/snapshot,t_1000,f_jpg,w_0,h_0,m_fast" alt="" width="500px" height="50px"></a>';
                            } else {
                                return '无';
                            }
                        }
                    ...
                    , {
                        field: 'undertaking', title: '承诺条款', align: 'center',
                        templet: function (d) {
                            if (d.undertaking) {
                                return '<div onclick="show_img(this)" ><img src="' + d.fill_undertaking + '" alt="" width="500px" height="50px"></a></div>';
                            } else {
                                return '无';
                            }
                        }
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '新增自拍认证'
                            , content: '{{url('system/member/user/selfie/create')}}'
                            , maxmin: true
                            , area: ['1000px', '800px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) {
                ...
                if (layEvent === 'audit') { //审核
                    layer.confirm('确认审核？', {
                        ...
                        axios.post("{{url('system/member/user/selfie/audit')}}", {
                            id: data.id,
                            status: 0
                        })
                ...
                if (layEvent === 'del') { //删除
                    layer.confirm('真的删除认证吗', function (index) {
                        axios.post("{{url('system/member/user/selfie/destroy')}}", {ids: [data.id]})
                            .then(function (response) {
                                    if (response.data.status) {
                                        layer.msg(response.data.msg);
                ...
                if (layEvent === 'edit') { //编辑
                    layer.open({
                        type: 2
                        , title: '编辑自拍认证'
                        , content: '{{url('system/member/user/selfie/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['1000px', '800px']
                        , end: function () {
                            tableIns.reload();
