        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <div class="layui-form-item">
                    ...
                    <div class="layui-inline">
                        <div class="layui-input-inline">
                            <input type="text" class="layui-input" name="dateTime" id="test" placeholder="上传时间"
                                   lay-key="17">
                        </div>
                    </div>
                    <div class="layui-inline">
    ...
    <script type="application/javascript">
        ...
        layui.use(['table'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('system/member/resource/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '会员资源'
                , cols: [[
                    ...
                    , {
                        ...
                        templet: function (d) {
                            if (d.file != null) {
                                if (d.type === 1) {
                                    return '<a href="' + d.file.url + '" target="_blank" title="点击查看"><img src="' + d.file.thumb + '?x-oss-process=video/snapshot,t_1000,f_jpg,w_0,h_0,m_fast" alt="" width="500px" height="50px"></a>';
                                } else {
                                    return '<div onclick="show_img(this)" ><img src="' + d.file.thumb + '" alt="" width="500px" height="50px"></div>';
                                }
                            } else {
                                return '';
                            }
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '新增会员资源'
                            , content: '{{url('system/member/resource/create')}}'
                            , maxmin: true
                            , area: ['800px', '400px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) {
                ...
                if (layEvent === 'audit') { //审核
                    layer.confirm('确认审核？', {
                        ...
                        layer.closeAll();
                        axios.post("{{url('system/member/resource/audit')}}", {
                            id: data.id,
                            status: 0
                        })
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    layer.confirm('确认删除？', function (index) {
                        axios.post("{{url('system/member/resource/destroy')}}", {ids: [data.id]})
                            .then(function (response) {
                                    if (response.data.status) {
                                        layer.msg(response.data.msg);
                    ...
                    layer.open({
                        type: 2
                        , title: '编辑会员资源'
                        , content: '{{url('system/member/resource/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['600px', '800px']
                        , end: function () {
                            tableIns.reload();
